...
const WatchComponent = ({ info, type, paramsId }: { info: SeriesInfo, type: string | undefined, paramsId: string }) => {
    ...
    // Use refs for AbortControllers to prevent memory leaks
    const controllerServersRef = useRef<AbortController | null>(null);

    // initialize Console Protection methods
    useEffect(() => {
    ...
    useEffect(() => {
        const setValues = (episodeNum: number) => {
            ...
            const validEpisodeNum = Math.max(1, Math.min(episodeNum, info.episodes.length));
        ...
        <div className={styles.watchContainer}>
            ...
            {loading ? (
                ...
                <div className={styles.contentWrapper}>
                    ...
                    <div className={styles.titleSection}>
                        ...
                        {selectedEpisodeNumber && (
                            <div className={styles.episodeIndicator}>
                                ...
                                <span className={styles.episodeNumber}>{selectedEpisodeNumber}</span>
                            </div>
                        )}
                    </div>
                        ...
                        <div className={styles.typeSelection}>
                            ...
                            <div className={styles.typeButtons}>
                                {servers.serversSub && servers.serversSub.length > 0 && (
                                    <button
                                        className={`${styles.typeBtn} ${subOrDub === 'sub' ? styles.active : ''}`}
                                ...
                                {servers.serversDub && servers.serversDub.length > 0 && (
                                    <button
                                        className={`${styles.typeBtn} ${subOrDub === 'dub' ? styles.active : ''}`}
                        ...
                        <div className={styles.episodesSection}>
                            ...
                            <div className={styles.episodeControls}>
                                <input
                                    ...
                                    style={{ width: '150px', marginRight: '0.5rem', padding: '0.5rem' }}
                                />
                                <span style={{ whiteSpace: "nowrap" }}>
                                    {groupIndex * groupSize + 1} to {Math.min((groupIndex + 1) * groupSize, info.totalItems)}
                                </span>
                            ...
                            <div className={`${styles.episodesGrid} ${seeMoreOrLess['episodes'] ? 'expandHeight' : ''}`} >
                                {groups[groupIndex]?.map((episode: Episode) => (
                                    <button
                                        key={episode.id}
                                        className={`${styles.episodeBtn} ${selectedEpisodeNumber === episode.number ? styles.active : ''}`}
                        ...
                        <div className={`${styles.metaGrid} ${seeMoreOrLess['meta'] ? 'expandHeight' : ''}`}>
                            {info.rate && (
                                <div className={styles.metaItem}>
                                    ...
                                    <span className={`${styles.metaValue} ${styles.rating}`}>{info.rate}</span>
                                </div>
                            )}
                            {info.quality && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>QUALITY</span>
                                    <span className={`${styles.metaValue} ${styles.quality}`}>{info.quality}</span>
                                </div>
                            )}
                            {info.pg && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>PARENTAL GUIDE</span>
                                    <span className={`${styles.metaValue} ${styles.pg}`}>{info.pg}</span>
                                </div>
                            )}
                            {info.info?.status && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>STATUS</span>
                                    <span className={`${styles.metaValue} ${styles.status}`}>{info.info.status}</span>
                                </div>
                            )}
                            {info.info?.aired && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>AIRED</span>
                                    <span className={`${styles.metaValue} ${styles.aired}`}>{info.info.aired}</span>
                                </div>
                            )}
                            {info.info?.duration && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>DURATION</span>
                                    <span className={`${styles.metaValue} ${styles.duration}`}>{info.info.duration}</span>
                                </div>
                            )}
                            {info.info?.synonyms && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>SYNONYMS</span>
                                    <span className={`${styles.metaValue} ${styles.studio}`}>{info.info.synonyms}</span>
                                </div>
                            )}
                            {info.info?.japanese && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>NAME (JAPANESE LANGUAGE)</span>
                                    <span className={`${styles.metaValue} ${styles.rating}`}>{info.info.japanese}</span>
                                </div>
                            )}
                            {info.info?.premiered && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>PREMIERED</span>
                                    <span className={`${styles.metaValue} ${styles.pg}`}>{info.info.premiered}</span>
                                </div>
                            )}
                            {info.info?.malScore && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>MAL SCORE</span>
                                    <span className={`${styles.metaValue} ${styles.status}`}>{info.info.malScore}</span>
                                </div>
                            )}
                            {info.totalItems && (
                                <div className={styles.metaItem}>
                                    <span className={styles.metaLabel}>EPISODES</span>
                                    <span className={`${styles.metaValue} ${styles.totalEpisodes}`}>{info.totalItems}</span>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    {/* Additional Info */}
                    {(info.info?.synonyms || info.info?.studios || info.info?.producers) && (
                        <div className={styles.additionalInfo}>
                            ...
                            {info.info?.producers && Array.isArray(info.info.producers) && (
                                <div className={styles.infoItem}>
                                    ...
                                    <p className={styles.infoValue}>{info.info.producers.join(', ')}</p>
                                </div>
                            )}
                        </div>
                    )}
            ...
            {info.recommended && info.recommended.length > 0 && (
                <div className={styles.recommendationsSection}>
                    ...
                    <div className={styles.recommendationsGrid}>
                        {info.recommended.slice(0, 12).map((rec: RecommendedItem, idx: number) => (
                            <Link href={`${process.env.NEXT_PUBLIC_BASE_URL}/watch${rec.animeId}`} key={idx} className={styles.recommendationCard} title={rec.title}>
                                <div className={styles.recImageContainer}>
                                    <Image src={rec.imageUrl} alt={rec.title} className={styles.recImage} width={300} height={350} />
